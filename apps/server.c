...
static char rsaCAFile[] = "2048_RSA_CA.pem";
	...
	if (alert > 0) {
		if (ALLOW_ANON_CONNECTIONS) {
			...
			if (cert) {
				...
				_psTrace("Client didn't send certificate. Reverting to standard handshake due toSERVER_WILL_ACCEPT_EMPTY_CLIENT_CERT_MSG\n");
			}   
